    ...
    for (let i = 0; i < slaMapping.length; i++) {
        ...
        if (createNew) {
            if (currentItem.value === 's_unit') {
                initialValues[currentItem.value] = s_unit;
            ...
            } else if (currentItem.value === 's_airline_code') {
                initialValues[currentItem.value] = s_airline_code;
            } else {
                initialValues[currentItem.value] = '';
            }
        } else {
            initialValues[currentItem.value] = updateSlaItem && updateSlaItem[currentItem.value];
        }
    }
                            ...
                            <Row className='pt-3'>
                                <Col md={12} className='mx-auto'>
                                    <Formik
                                        ...
                                        validate={values => {
                                            const errors = {};
                                            const exclude = ['id', 't_modified', 's_modified_by'];
                                        ...
                                        {({ isSubmitting, isValid, errors, values, setFieldValue }) => (
                                            <Form>
                                                <FormGroup>
                                                    ...
                                                    <Input value={values.s_unit} />
                                                </FormGroup>
                                                <FormGroup>
                                                    <Label className='mr-2'>Airline Name</Label>
                                                    <Input value={values.s_airline_code} />
                                                </FormGroup>
                                                <FormGroup>
                                                ...
                                                {
                                                    !createNew && 
                                                    <Button className='ml-2' color='danger' onClick={() => deleteSla(updateSlaItem.id)}>
                                                        Delete
                                                    </Button>
                                                }
                                            </Form>
