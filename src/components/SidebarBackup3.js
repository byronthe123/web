...
class Sidebar extends Component {
    ...
    getIsHasSubItem = () => {
        const { selectedParentMenu, accessMap } = this.state;
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if (prevState.location.pathname !== nextProps.location.pathname) {
            updates.location = nextProps.location;
            console.log(`prev: ${prevState.location.pathname}, next: ${nextProps.location.pathname}`);
        }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.state.pathname !== this.props.location.pathname) {
            this.setState({
                ...
                const { pathname, accessMap } = this.state;
                for (let tabId in accessMap) {
                    ...
                    for (let sub in subs) {
                        if (subs[sub].to === pathname) {
                            ...
                            this.setState({
                                selectedPageId: accessMap[tabId].id
                            }, () => {
                        ...
                        for (let finalSubId in finalSubs) {
                            if (finalSubs[finalSubId] === pathname) {
                                this.setState({
                                    selectedPageId: accessMap[tabId].id
                                }, () => {
                                    console.log(this.state.selectedPageId)
                                                    ...
                                                    (key, i) => (
                                                        <NavItem
                                                            ...
                                                            className={classnames(
                                                                {
                                                                    active: sidebar[key].id === selectedPageId
                                                                }
                                                            )}
                                                        >
                                                            <NavLink
