...
export default function UploadFiles<T>({
    ...
    const getImageDimensions = (imgData: any) => {
        console.log(imgData);
        return new Promise((resolve, reject) => {
            ...
            image.onload = function() {
                ...
                // @ts-ignore
                URL.revokeObjectURL(this.src);  // Important: release object URL to avoid memory leaks
                resolve({
                    // @ts-ignore
                    imgWidthPx: this.naturalWidth, imgHeightPx: this.naturalHeight
                });
            };
    ...
    const handleSave = async () => {
        ...
        if (multiple) {
            ...
            // size of margins (in mm)
            const margin = 10;
            const marginPt = (margin * 72) / 25.4; // Convert mm to points
            const dpi = 300;
            ...
            for (let index = 0; index < files.length; index++) {
                ...
                // @ts-ignore
                const { imgWidthPx, imgHeightPx } = await getImageDimensions(imgData);
                console.log(imgWidthPx, imgHeightPx);

                const imgWidthIn = imgWidthPx / dpi;
                const imgHeightIn = imgHeightPx / dpi;

                // convert inches to points
                const imgWidthPt = imgWidthIn * 72 - marginPt;
                const imgHeightPt = (imgWidthPt / imgWidthIn) * imgHeightIn;
        
                // calculate the width and height of the image in the PDF (in mm)
                let imgWidthMm = imgWidthPx / 28.35;  // roughly converting pixels to mm
                let imgHeightMm = imgHeightPx / 28.35;  // roughly converting pixels to mm

                doc.addImage(imgData, 'JPEG', margin, margin, imgWidthPt, imgHeightPt);
            }
