...
const Sidebar = (props) => {
    ...
    useEffect(() => {
        console.log(`Sidebar props changed`);
        console.log(props);
    }, [props]);
    ...
    const [overdueReadingSign, setOverdueReadingSign] = useState(false);
    ...
    useEffect(() => {
        ...
        for (let tabId in accessMap) {
            const { subs } =  accessMap[tabId];
            for (let sub in subs) {
                if (subs[sub].to === pathname) {
                    console.log(`pathname = ${pathname}, subs[sub].to = ${subs[sub].to}, tabId = ${tabId}`);
                    setSelectedParentMenu(accessMap[tabId].id);
                    setSelectedPageId(accessMap[tabId].id);
                    setSelectedSubId(subs[sub].id);
                    break;
                }
                const finalSubs = subs[sub].subs;
                for (let finalSubId in finalSubs) {
                    if (finalSubs[finalSubId].to === pathname) {
                        setSelectedParentMenu(accessMap[tabId].id);
                        setSelectedPageId(accessMap[tabId].id);
                        setSelectedSubId(subs[sub].id);
                        break;
    ...
    }, [props.accessMap, props.location.pathname]);

    const getContainer = () => {
        return domNodeRef && domNodeRef.current;
    };
    ...
    const setSelectedLiActive = (callback) => {
        ...
        if (selectedlink != null) {
            ...
            if (selectedParentNoSubItem != null) {
                setSelectedParentMenu(selectedParentNoSubItem.getAttribute('data-flag'));
                callback();
    ...
    const getIsHasSubItem = () => {
        let menuItem;
        for (let tabId in accessMap) {
            if (accessMap[tabId].id === selectedParentMenu) {
                menuItem = accessMap[tabId];
            }
        }
        if (menuItem)
            if (menuItem && menuItem.subs && Object.keys(menuItem.subs).length > 0) {
                return true;
            } else {
    ...
    const toggleMenuCollapse = (e, menuKey) => {
        ...
        if (collapsedMenus.indexOf(menuKey) > -1) {
            ...
            setCollapsedMenus(prev => [...prev, menuKey]);
        }
        return false;
    };
    ...
    const minimizeSubmenu = () => {
        ...
        const { containerClassnames, menuClickCount } = props;
        props.setContainerClassnames(1, containerClassnames, false);
    };

    const { containerClassnames, accessMap, accessMapAssigned } = props;
    const currentClasses = containerClassnames
        ? containerClassnames.split(' ').filter((x) => x !== '')
        : '';
    ...
    const isActive = (selectedParentMenu, viewingParentMenu, item) => {
        if (
            (selectedPageId === item.id && viewingParentMenu === '') ||
    ...
    return (
        <div
            className={`sidebar ${
                overdueReadingSign && 'customDisabled'
            }`}
                                ...
                                <Nav vertical className="list-unstyled">
                                    {
                                        Object.keys(accessMap).map(
                                            (key, i) => sidebar[key].label && (
                                                <NavItem
                                                    key={sidebar[key].id}
                                                    className={classnames({
                                                        active:
                                                        (selectedPageId === sidebar[key].id &&
                                                            viewingParentMenu === '') ||
                                            ...
                                            Object.keys(
                                                ...
                                                >
                                                    {accessMap[key].subs[sub].subs &&
                                                        ...
                                                        <Fragment>
                                                            <NavLink
                                                                className={`rotate-arrow-icon ${selectedSubId === accessMap[key].subs[sub].id && 'text-primary'} ${darkModeTextClass} ${
                                                            ...
                                                            >
                                                                <i className="simple-icon-arrow-down" style={{ color: 'inherit' }} />
                                                                {
                                                        ...
                                                        <NavLink
                                                            ...
                                                            className={classnames(
                                                                { 'text-primary': selectedSubId === accessMap[key].subs[sub].id }
                                                            )}
                                                        >
                                                            <i
