...
const TruckFlights = ({
    ...
    const s_destination = station && station;
    ...
    useEffect(() => {
        if (s_mawb_prefix.length === 3) {
            ...
            axios.post(`${baseApiUrl}/resolveIsc`, {
                ...
                if (response.data[0]) {
                    set_f_daily_storage_rate(resolveIscCost(response.data[0]));
                    set_s_airline_code(response.data[0].s_airline_code);
                } 
            }).catch(error => {
    ...
    return (
        <AppLayout>
            <div className={`card queue-card-container`} style={{backgroundImage: 'url(/assets/img/bg-misc-1.jpg)', backgroundSize: 'cover', backgroundRepeat: 'no-repeat', backgroundPosition: 'center'}}>
...
export default withRouter(TruckFlights);